...
class CrudLaundryModel extends CI_Model
{
    public function generateCode($code, $field, $table)
    {
        $automatic_code = $this->db->select_max($field)->get($table);
        $maxCode = (int) substr($automatic_code->row()->$field, 2, 6) + 1;
        $addCode = $code.sprintf('%06s', $maxCode);
